#python Run_1.py --interface wlan1 --hop --channels 1,6,11 --interval 1.0
#!/usr/bin/env python3
"""
@Tobias Jensen
@date: 22/4/2026
...
def channel_hop_thread(interface: str, channels: list[int], interval: float):
    """
    Hopper mellem Wi-Fi kanaler i en separat tråd, så sniffing kan fange frames på flere kanaler.
    """
    idx = 0
...
def main():
    ...
    parser.add_argument("--hop", action="store_true",
                        help="Enable channel hopping across a list of channels")
    parser.add_argument("--channels", default="1,6,11",
                        help="Comma-separated list of channels to hop (default: 1,6,11)")
    parser.add_argument("--interval", type=float, default=1.0,
                        help="Channel hop interval in seconds (default: 1.0)")
    ...
    if args.hop:
        channels = [int(ch.strip()) for ch in args.channels.split(",") if ch.strip()]
        if not channels:
            channels = [1, 6, 11]
        print(f"[*] Starting channel hopping on {args.interface} with channels: {channels} and interval {args.interval}s")
        threading.Thread(target=channel_hop_thread, args=(args.interface, channels, args.interval), daemon=True).start()
    elif args.channel:
