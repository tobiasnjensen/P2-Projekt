#!/usr/bin/env python3
#ved ikke hvad det her gør men læste man skal bruge det for at køre på PI
"""
@Tobias Jensen
@date: 22/4/2026
RSSI-måler der kører på Raspberry Pi'erne. Sniffer Wi-Fi-pakker, udtrækker RSSI, estimerer afstand og sender data til serveren via UDP.
Dele af koden kan godt være lidt uoverskueligt, men den overordnede ide er at den måler en RSSI værdi fra en enhed og konverterer til afstand.
Hvorefter den sendes til serveren som en JSON-fil på følgende format:
{
    "bssid": "string",      
    "anchor_id": "string",  #Identifikator for hvilken Raspberry Pi der har lavet målingen, f.eks. "anchor_4"
    "distance": float,  
}
...
#--------Diverse variabler--------#
RTL8187_OFFSET = 0 #Korrektionsoffset for RTL8187-adapteren, som kan justeres baseret på kalibrering men virker ikke nødvendig alligevel
...
def rssi_to_distance(rssi, tx_power=-18.7, path_loss_exp=2.5) -> float:
    ...
    tx_power: Kalibreret RSSI-værdi ved 1 meters afstand (A), standard -17.1 dBm
...
def bring_interface_up(interface: str) -> bool:
    ...
    Dog fungerer det ikke altid og skal nogle gange gøres manuelt :|
    Skriver basicly bare de linux commands der skal skrives for at få interface i monitor mode
    args:
    ...
    try:
        ...
        return True
    except subprocess.CalledProcessError as e: #Hvis interfacet ikke kunne genstartes printes en fejlmeddelelse
...
def get_ssid(packet) -> str: #Gammel funktion der ikke bruges men tør ikke slette den. <---
    """
    Funktion til at udtrække SSID-navnet fra en Beacon- eller ProbeResponse-pakke. F.eks. 
    args:
    packet: Scapy-pakken
    returns:
...
def make_packet_handler(mac_filter: str | None, udp_sock, target_address):
    ...
    """
    mac_filter_normalized = mac_filter.lower() if mac_filter else None #Laver MAC-filteret til lowercase

    rssi_history: dict[str, collections.deque] = {} #Dictionary der gemmer en rolling RSSI-historik per BSSID

    def packet_handler(packet) -> None:
        """
        Indre funktion der behandler en modtaget Wi-Fi-pakke.
        Udtrækker RSSI, beregner rolling average, estimerer afstand og sender data via UDP.
        args:
        packet: Scapy-pakken der skal behandles
        """
        rssi = get_rssi(packet) #Udtrækker RSSI-værdien fra pakken og gemmer i rssi variablen
        ...
        if not packet.haslayer(Dot11): #Hvis pakken ikke har et Dot11-lag, stoppes funktionen.
            return #Dot11-laget indeholder de relevante MAC-adresser
        ...
        udp_sock.sendto( #Sender en JSON-pakke med måledata til serveren via UDP
            json.dumps({
                ...
                "anchor_id": "anchor_4"   #Identifikator for denne Raspberry Pi (hardcoded per Pi)
            }).encode('utf-8'),
...
def main():
    ...
    parser.add_argument("--window", type=int, default=ROLLING_WINDOW, #NOTE: Jeg glemte det har var en ting så brug inputtet i stedet for --window
    ...
    print(f"{'Type':<8}   {'Address':<20}   {'SSID/DST':<32}   {'RSSI':<12} {'Avg RSSI'}")
    print("-" * 110)
