...
@Date: 29/4/2026
Kalibrering af RSSI. Skal bruges til at finde reference RSSI værdien til afstandsestimat
Koden er stort set klippe klistret fra Run_? filerne
Howto:
sudo python3 Calibration.py -i wlan1 --mac aa:bb:cc:dd:ee:ff -c 6/11/idk
"""
...
#--------Signal Håndtering--------#
def handle_sigint(sig, frame)-> None: #Funktion til at stoppe kalibreringen tidligt ved Ctrl+C
    global stop
    print("\n[*] Stopping calibration early…")
    stop = True

#--------Sniffing stuff--------#
def set_channel(interface, channel)-> None:
    """
...
def packet_handler(packet, mac_filter)-> None:
    ...
    rssi = get_rssi(packet) #Gemmer målt RSSI-værdi i rssi variablen
