...
positions = {} # { bssid: {"position": (x, y, z), "timestamp": "...", "count": N} }
...
#--------Trilateration--------#
def try_trilaterate(bssid, timestamp):
    """
    ...
    args:
        ...
        timestamp (str): Tidsstempel fra målingen.
        Process:
        ...
        4. Kalder tri_lat funktionen for at estimere positionen.
...
#Unlocker låsen så trilateration kan læse measurements uden at blokere receiver-tråden
    result = tri_lat( #Kalder trilateration funktionen med de kendte anchor-koordinater og de målte afstande
    ...
    if result is None: #BURDE ALDRIG SKE, DA TRILAT BRUGER NLLS. Det er skrevet før NLLS blev implementeret
        ...
        print(f"[{bssid}] Position: x={result[0]:.2f}, y={result[1]:.2f}, z={result[2]:.2f} (måling #{positions[bssid]['count']})")
...
def worker_thread():
    ...
    while True:
        ...
        print(f"[{parsed['timestamp']}] {bssid} fra {anchor_id} → {distance:.2f}m") #Printer timestamp, BSSID, anchor_id og dens afstand til objektet
        try_trilaterate(bssid, parsed['timestamp']) #Forsøger at udføre trilateration for den givne BSSID, hvis der ikke er nok måleenheder endnu, vil den blot udskrive hvilke der mangler
