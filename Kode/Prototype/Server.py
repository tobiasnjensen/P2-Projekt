...
client_slots = {} #Dict med slots

#----------------------------------#
#            Functions             #
#----------------------------------#

def rssi_to_distance(rssi, A, n) -> float:
    """
    Estimerer afstanden baseret på RSSI-værdien
    
    Parameters:
    rssi (float): The received signal strength indicator (in dBm).
    A (float): Reference RSSI værdi ved 1 meter. Den er bare sat til -40dBm uden nogen specifik måling
    n (float): Path-loss exponent, som afhænger af miljøet. En typisk værdi for indendørs miljøer er 2,7.
    
    Returns:
    float: Estimeret afstand i meter.
    """
